...
def _autonomous_browse(
    ...
    system_prompt = (
        ...
        "  Пример: сначала он просит «найди свободный слот на завтра»,\n"
        "  затем пишет «забронируй его и подготовь оплату». Это одна сессия работы.\n"
        "- Если текущая страница уже подходит (открыт нужный сервис/корзина/календарь),\n"
        ...
        "- Если цель похожа на уточнение/дополнение («поменяй дату», «добавь ещё час»),\n"
        "  продолжай работать в рамках текущего сервиса и уже выбранного слота.\n"
        ...
        "\n"
        "Работа с арендой:\n"
        "- Поиск: используй поля поиска, фильтры по датам и времени, чтобы показать свободные варианты.\n"
        "- Бронь: убедись, что выбран правильный слот (дата/время/локация), заполни обязательные поля, но остановись перед окончательной отправкой.\n"
        "- Оплата: проверь корзину/счёт, зафиксируй сумму и условия, но НЕ нажимай финальные кнопки оплаты без явного подтверждения пользователя.\n"
        "\n"
