...
BROWSER_ACTION_RULES = join_bullets(
    [
        ...
        "- Для поиска используй ввод текста с запросом вроде «поиск»/«search»: если задан SEARCH_URL_TEMPLATE",
        "  и SEARCH_URL_MODE=url_template/auto, агент подставит текст прямо в URL (шаблон с {query})",
        "  и откроет страницу без поиска поля (например, Google/Yandex).",
